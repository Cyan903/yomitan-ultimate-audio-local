...
router.get("/list", withApiKey, async (request: IRequest, env: Env) => {
    ...
    return json(await generateYomitanResponseObject(sortedResults, ttsEntries, request, env), { status: 200 });
});
...
router.get("/get/:source/:file", withApiKey, async (request: IRequest, env: Env) => {
    ...
    log("info", "audio_get", `Fetching audio: ${source}/${file}`, {
        source,
        file,
    });

    return mp3(await returnAudio(source, file), { status: 200 });
});
