 ...
 */
const KATAKANA_CHART = "ァアィイゥウェエォオカガカ゚キギキ゚クグク゚ケゲケ゚コゴコ゚サザシジスズセゼソゾタダチヂッツヅテデトドナニヌネノハバパヒビピフブプヘベペホボポマミムメモャヤュユョヨラリルレロヮワヰヱヲンヴヵヶヽヾ";
const HIRAGANA_CHART = "ぁあぃいぅうぇえぉおかがか゚きぎき゚くぐく゚けげけ゚こごこ゚さざしじすずせぜそぞただちぢっつづてでとどなにぬねのはばぱひびぴふぶぷへべぺほぼぽまみむめもゃやゅゆょよらりるれろゎわゐゑをんゔゕゖゝゞ";
...
async function generateVariantsFromChars(chars: string[], index: number): Promise<string[]> {
    ...
    if (index < chars.length - 1) {
        ...
        if (chars[index + 1] === "ウ" && (current === "オ" || current === "コ" || current === "ソ" || current === "ト" || current === "ノ" || current === "ホ" || current === "モ" || current === "ロ" || current === "ゴ" || current === "ゾ" || current === "ド" || current === "ボ" || current === "ポ" || current === "ヨ" || current === "ショ" || current === "チョ" || current === "ジョ")) {
        ...
        if (chars[index + 1] === "イ" && (current === "エ" || current === "ケ" || current === "セ" || current === "テ" || current === "ネ" || current === "ヘ" || current === "メ" || current === "レ" || current === "ゲ" || current === "ゼ" || current === "デ" || current === "ベ" || current === "ペ")) {
