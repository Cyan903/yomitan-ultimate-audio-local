...
const VALID_AUDIO_SOURCES = ["all", "nhk16", "daijisen", "shinmeikai8", "jpod", "taas", "ozk5", "forvo", "forvo_ext", "forvo_ext2", "tts"] as const;
...
export async function unpack_pitch(request: IRequest): Promise<string> {
    ...
    return (rawPitch === undefined || rawPitch.length === 0) ? "" : rawPitch;
}
